...
function buildIframeUrl(cfg: Config): string {
  ...
  params.set("kiosk", "");
  return `${GRAFANA_BASE_URL}/d-solo/${cfg.dashboardId}?${params.toString()}`;
}
      ...
      <div className="flex h-full flex-col overflow-auto">
        <div className="flex items-center justify-between border-b border-border px-4 py-3">
          <div className="flex items-center gap-2">
            ...
            <span className="text-sm font-medium">Panel Grafana</span>
          </div>
          {isConfigured && (
        ...
        <div className="flex-1 space-y-3 overflow-auto p-4">
          ...
          <div className="space-y-1">
            <label className="text-xs font-medium text-muted-foreground">Panel ID</label>
            <input type="number" value={form.panelId} onChange={(e) => setForm((f) => ({ ...f, panelId: e.target.value }))} placeholder="2" className="w-full rounded-md border border-border bg-background px-3 py-1.5 text-sm outline-none focus:ring-1 focus:ring-primary" />
          </div>
          <div className="space-y-1">
            <label className="text-xs font-medium text-muted-foreground">Titre (optionnel)</label>
            <input type="text" value={form.title} onChange={(e) => setForm((f) => ({ ...f, title: e.target.value }))} placeholder="CPU usage" className="w-full rounded-md border border-border bg-background px-3 py-1.5 text-sm outline-none focus:ring-1 focus:ring-primary" />
          </div>
          <div className="grid grid-cols-2 gap-3">
            <div className="space-y-1">
              <label className="text-xs font-medium text-muted-foreground">Rafraîchissement</label>
              <select value={form.refreshInterval} onChange={(e) => setForm((f) => ({ ...f, refreshInterval: e.target.value }))} className="w-full rounded-md border border-border bg-background px-3 py-1.5 text-sm outline-none focus:ring-1 focus:ring-primary">
          ...
          <button onClick={handleSave} disabled={!form.dashboardId || !form.panelId} className="w-full rounded-md bg-primary px-4 py-2 text-sm font-medium text-primary-foreground disabled:opacity-50">
            Sauvegarder
          </button>
        </div>
      </div>
  ...
  return (
    <div className="group relative h-full w-full">
      ...
      <div className="absolute right-2 top-2 z-10 flex items-center gap-1 opacity-0 transition-opacity group-hover:opacity-100">
        ...
        {iframeUrl && (
          <a href={iframeUrl.replace("/d-solo/", "/d/").split("?")[0]} target="_blank" rel="noopener noreferrer" className="rounded bg-black/50 p-1 text-white backdrop-blur-sm hover:bg-black/70" aria-label="Ouvrir dans Grafana">
      ...
      {iframeUrl && (
        <iframe key={refreshKey} src={iframeUrl} className="h-full w-full border-0" onLoad={() => setIsIframeLoading(false)} onError={() => { setIsIframeLoading(false); setHasError(true); }} title={form.title || "Grafana Panel"} />
      )}
    </div>
  );
}
